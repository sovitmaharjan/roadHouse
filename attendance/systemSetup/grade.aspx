<%@ Page Title="" Language="C#" MasterPageFile="~/layout.Master" AutoEventWireup="true" CodeBehind="grade.aspx.cs" Inherits="attendance.systemSetup.grade" %>
                ...
                <form id="form" class="form-horizontal" runat="server">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input id="gradeName" type="text" class="form-control" runat="server" autocomplete="off" required="required">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-2 control-label">
                                Grade Type<span class="text-danger">*</span>
                            </label>
                            <div class="col-md-9">
                                <input id="gradeType" type="text" class="form-control" runat="server" autocomplete="off" required="required">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-2 control-label">
        ...
        $(document).ready(function () {
            dataTableFunction(function () {
                $('.edit').on('click', function () {
                    ...
                    $.ajax({
                        method: 'post',
                        url: '<%=this.baseUrl%>systemSetup/grade.aspx/getData',
                        data: '{ "id": ' + id + '}',
                        contentType: "application/json; charset=utf-8",
                        dataType: 'json',
                        success: function (result) {
                            result = result.d[0];
                            $('#<%=id.ClientID%>').val(result['GRADE_ID']);
                            $('#<%=gradeName.ClientID%>').val(result['GRADE_NAME']);
                            $('#<%=gradeType.ClientID%>').val(result['GRADE_TYPE']);
                            if (result['sta'] == 1) {
            ...
            $('#add').on('click', function () {
                $('#<%=id.ClientID%>').val('');
                $('#<%=gradeName.ClientID%>').val('');
                $('#<%=gradeType.ClientID%>').val('');
                $('#<%=statusYes.ClientID%>').prop('checked', true);
                $('#<%=statusNo.ClientID%>').prop('checked', false);
            })
